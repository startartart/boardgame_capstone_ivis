      ...
      {props.direction == 'left' ? (
        <div className="left-center">
          <div className="dialog">
            {select_card == 3 && props.area == 'enemy' ? (
              <GuessText>. . . .</GuessText>
            ) : (
              <>
                <GuessLabel
                  src={`./images/poker_cards/${props.answer}.png`}
                  alt="guess-emoji"
                />
              </>
            )}
          ...
          {select != -1 && (
            <div className="right-center">
              <div className="dialog">
                {select_card == 1 ? (
                  <GuessText>
                    이미지 혹은 동물 이름을 클릭하면 Guess 완료!
                  </GuessText>
                ) : (
                  <>
                    {select_card == 2 ? (
                      <GuessText>시간 내에 답변하세요!</GuessText>
                    ) : (
                      <>
                        {select_card == 3 ? (
                          <GuessText>상대방의 답변을 기다리는 중 ..</GuessText>
                        ) : (
                          <>
                            {guess <= 2 && (
                              <GuessSelect onClick={SelectGuessHandler}>
                                - 추측하기 ({guess}/3)
                              </GuessSelect>
                            )}
                            {guess >= 1 && (
                              <GuessSelect onClick={SelectDecideHandler}>
                                - 이 카드로 결정
                              </GuessSelect>
                            )}
                          </>

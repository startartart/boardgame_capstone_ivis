...
import {
  useJokerGameState,
  useJokerGameDispatch,
} from '../../../contexts/JokerGameContext';
import {
  PeekSocketEvent,
  SelectSocketEvent,
  ListenJokerGameSocketEvents,
} from '../../../events/JokerGameSocket';
...
const Board = (props) => {
    ...
    for (let i = 0; i < value; i++) {
      if (i == index) {
        ...
        if (jokerGameState.myHand[i] == 'joker') {
          arr.push(
            <Card
              src={`./images/cards/${jokerGameState.myHand[i]}${props.theme.mode}.png`}
              alt="card"
              key={i}
          ...
          arr.push(
            <Card
              src={`./images/cards/${jokerGameState.myHand[i]}.png`}
              alt="card"
              key={i}
              onClick={() => cardCheatHandler(i)}
              peeked={true}
  ...
  return (
    <BoardContainer theme={props.theme}>
      <CardBox>
        <GameText>상대 카드</GameText>
        {ShowEnemyCard(jokerGameState.enemyHand, current)}
      </CardBox>

      {jokerGameState.expression == './images/emoji/emoji0.png' ? (
        <GameText>얼굴을 제대로 인식해주세요</GameText>
      ) : (
        <GameText>조커 뽑기</GameText>
      )}
